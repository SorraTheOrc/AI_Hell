...
> **Bullet range — per-type lifetime + four-edge wrap:** All bullets (player and enemy) **wrap around all four screen edges** (the same classic Asteroids model as the player ship and asteroids) and are **never culled for leaving the screen**. Each bullet type is instead destroyed once its own **lifetime in seconds** elapses, so its **effective range is `bulletSpeed × lifetime`**. Player-weapon lifetimes live on `WeaponDefinition` (`src/utils/weapons.ts`): Cannon **3.0 s** (~1050 px at 350 px/s), Spread **2.8 s** (~980 px), Dual **2.8 s** (~980 px), Rapid **1.5 s** (~525 px). Enemy bullet lifetimes live on `EnemyConfig` (`src/core/enemyConfig.ts`) and are overridable through the existing enemy-config / localStorage plumbing: Scout **3.0 s** (200 px/s ≈ 600 px), Diver **3.0 s** (220 px/s ≈ 660 px), Tank **4.0 s** (150 px/s ≈ 600 px), Phaser **3.5 s** (180 px/s ≈ 630 px), Swarm **3.0 s** (180 px/s ≈ 540 px), Boss Swarm **4.0 s** (160 px/s ≈ 640 px). The Central AI boss uses the same 4.0 s default. Because wrapping keeps more bullets alive, the lifetime caps on-screen density; the values above are the shipped tuning baseline and are safe to adjust without any architectural change.
