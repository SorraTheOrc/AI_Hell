...
- **Files:** `src/data/enemy-configs.csv` (one row per enemy) and `src/data/ship-config.csv` (one row for the player ship). Both files carry a `#` comment header listing every column, the enum values and how to add entries.
...
- **Boot:** at startup the config store (`src/core/configStore.ts`) reads both CSVs, parses/validates them with the CSV codec (`src/core/csv.ts`) and populates an in-memory registry, so `loadEnemyConfig` / `loadShipConfig` stay synchronous. A missing/malformed file falls back to the built-in defaults without throwing.
- **Editing by hand:** edit a cell in `src/data/enemy-configs.csv` or `src/data/ship-config.csv`, save the file, and refresh `npm run dev` — the new values are live.
