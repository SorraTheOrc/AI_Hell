...
- `src/audio/effects.ts` — procedural WebAudio sound cues (spawn blip, destruction burst, Scout-specific fire sound and a ≥ 500 ms firing advance cue, Tank cue+thump, Swarm burst, Diver fire sound and distinct Diver destruction sound), all safe no-ops when no audio context exists (e.g. headless tests). Scout fires announce each aimed shot with a per-entity advance cue: the cue and the fire sound are both scheduled at tell start, back-to-back, with the fire sound landing exactly at the cue's end (no gap between cue and shot sound). Diver plays `playDiverFireSound()` once per spread burst (no advance cue) and `playDiverDestructionSound()` via the optional per-entity `playDestructionAudio?()` seam on `GymFormationScene` (once per destruction, no double-play). Destruction otherwise reuses the shared destruction sound played by the base `GymFormationScene` (no double-play).

The scene is reachable from the gym index ("Scout" entry) and returns to it via the "← INDEX" button — it is a gym testbed: the keyboard-controlled player ship (arrows + WASD, honouring the player's saved control scheme — under the asteroids scheme `W`/Up thrust ahead in the current facing, `A`/Left and `S`/Right turn) is present with live combat (player bullets destroy scouts; scout hits trigger explosion + respawn, infinite lives), but there is no HUD score, no power-ups, and no other enemies. Coverage: `src/entities/Scout.test.ts` + `src/scenes/gym/GymScout.test.ts` verify formation geometry, movement, explosion, shoot toggle and aimed-bullet behaviour.
