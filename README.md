...
The scene is no longer listed directly on the gym index — the index's **Boss** ENEMIES row boots the reusable `GymEnemies` scene with `enemyKey: 'boss'`, while `GymBoss` is excluded from the plain scene list so the boss is not listed twice (AH-0MUAYB28C004KK7X); `GymBoss` remains available programmatically and via its own tests. Coverage: `src/scenes/gym/GymBoss.test.ts` verifies scene/entity presence, visual style, health-bar segments, phase transitions via the DAMAGE button, all four phase attack patterns (including desperation combining patterns), telegraph timing ≥ 500 ms, and destruction/explosion.
