...
> **Shared combat core (`src/scenes/core/CombatCoreScene.ts` → `src/scenes/core/CombatScene.ts`)** — the shared combat/lifecycle logic lives in a two-tier hierarchy so no production scene keeps a divergent copy. `CombatCoreScene` is the narrower base: it owns the input path (`_readPlayerInput`), auto-fire (`_autoFire` + `spawnPlayerBullet`, with the `onWeaponFired` cue hook) and drop collection (`_collectDrop` + absorb VFX, with the `onWeaponCollected`/`onPowerUpCollected`/`_playPickupCue` hooks), the player-explosion/collect registries, `_clearEnemyBullets`/`_spawnPlayerExplosion`, and the shared invulnerability/phase/absorption hooks (`getInvulnerabilityDuration`, `isPlayerPhased`, `tryAbsorbPlayerHit`). The combat-specific `CombatScene` extends it and adds collision resolution, hostile hits and teleports (`_handleCollisions`, `_hitPlayer`, `_handleTeleport`/`triggerTeleport`, plus the bullet-vs-bullet impact feedback). Concrete scenes extend whichever tier they need and supply scene specifics through the overridable hook contract (participant accessors plus `onWeaponFired`, `onEnemyDestroyed`, `onPlayerHit`, `onPowerUpCollected`, `canTeleport`, `onBulletVsBulletImpact`, …). Hierarchy: `CombatCoreScene` → `CombatScene` → `PlayScene` / `GymFormationScene` / `GymPowerUpsCombat`; and `CombatCoreScene` → `GymWeapons` / `GymPowerUpsUtility`. Each of the eight shared template methods (`_handleCollisions`, `_hitPlayer`, `_autoFire`, `_collectDrop`, `_spawnPlayerExplosion`, `_clearEnemyBullets`, `_handleTeleport`, `_readPlayerInput`) is defined exactly once across all production scenes, enforced repo-wide by `CombatScene.equivalence.test.ts`. Bullet-vs-bullet interceptions get a dedicated `playBulletDestructionSound()` cue and a small impact flash (`src/vfx/bulletImpact.ts`) from the single shared path (AH-0MUD8E015004C4JO; standalone-gym consolidation AH-0MUDCT7EU0061OSZ).
>
> **Shared restart/teardown lifecycle (`CombatCoreScene.resetRunState()` / `teardownRunState()`)** — the per-run reset and teardown are owned once by the shared core so every scene starts a restart from the same clean state (AH-0MUII3FYN0072QRT, gap 10). `create()` calls `resetRunState()`, which clears the active `EffectsRegistry` through the polymorphic `getEffectsRegistry()` accessor (so a scene that owns its own registry is cleared by the same code as the game) plus the shared bullet/effect/animation registries; the scene's `SHUTDOWN` handler calls `teardownRunState()`, which destroys those registries and resets the effects registry. Scenes override both and call `super` to add their own object families (enemies, drops, minerals, player, HUD). `PlayScene` and every gym (`GymFormationScene` and its `GymEnemies`/`GymBoss`/`GymMinerals` subclasses, `GymWeapons`, `GymPowerUpsCombat`, `GymPowerUpsUtility`, `GymPlayer`) consume it — no scene keeps a divergent reset/teardown copy.

> **Shared projectile lifecycle (`src/scenes/core/bulletLifecycle.ts`)** — enemy-bullet advance + four-edge wrap + lifetime expiry is owned once by `advanceWrappingBullets(bullets, dt, width, height)`, and player-bullet advancement by `advancePlayerBullets(bullets, dt)` (delegating to the existing `advanceAndCull`). Consumed by `PlayScene`, `GymFormationScene`, `GymWeapons` and `GymPowerUpsCombat`, so the wrap/expiry semantics cannot drift (AH-0MUII3CF00024EDM; bullets are never culled off-screen, they wrap and expire by lifetime — AH-0MU960UTE001PTV0). Pinned by the repo-wide guard and the cross-scene equivalence tests in `CombatScene.equivalence.test.ts`.

> **Shared enemy-fire dispatch (`src/entities/enemyFire.ts`)** — the archetype-key → `tryFire*` mapping is owned once by `fireForEnemy(entity, enemyKey, now)`, which reads a single `ENEMY_FIRE_METHODS` table, normalises a `null`/single/array result to a bullet array and falls back to the aimed shot for unknown/custom keys. Consumed by `PlayScene`, `GymEnemies` and `GymPowerUpsCombat`, so a new enemy archetype is wired once and every scene fires it with the same cadence. The helper takes the scene clock as an explicit `now` argument — never a frame-count accumulator — so the combat gym's fire timing matches the game's (AH-0MUII3BBW000XZ46, gap 2). Pinned by the repo-wide guard in `CombatScene.equivalence.test.ts` and the dispatch tests in `src/entities/enemyFire.test.ts`.
...
1. **Tune in the gym:** `npm run dev` → **Gym Index → any Enemies entry** (e.g. Scout). Use the **Enemies panel** (`enemy-gym-panel`) sliders / colour pickers / selects — changes live-apply without reload. The panel's **live difficulty readout** (`enemy-gym-difficulty`) shows the edited archetype's absolute 0–100 score (see `docs/ENEMY_DESIGN_AND_IMPLEMENTATION.md` §9). Every gym panel (`gym-config-panel`, `enemy-gym-panel`, `boss-gym-panel`) is anchored **bottom-left** with a viewport-relative `max-height` + internal scroll, keeping the top-left HUD (lives/power-ups/minerals) visible; the in-canvas `EXPLODE`/`SHOOT`/`DAMAGE` controls sit **bottom-right**. A shared **collapse control** (`.gym-panel-toggle`) collapses each panel to its header so the whole scene is visible; panels start expanded and the state is per-scene (not persisted) (see `docs/ENEMY_DESIGN_AND_IMPLEMENTATION.md` §8.5).
...
- **Player-death juice:** the player's destruction composes additional layers on top of the particle burst through one shared entry point, `spawnPlayerDeathJuice(scene, x, y, severity, options?)` in `src/vfx/playerDeathJuice.ts` — camera shake (`applyShake`), a dedicated hull-breach SFX (`playPlayerDestructionSound`), the delegated particle burst, a full-screen flash (`spawnDeathFlash`), debris shards (`spawnDeathDebris`) and an expanding shockwave ring (`spawnDeathShockwave`). `resolveJuiceParams(severity)` maps `'respawn'` (mid-run) vs `'fatal'` (run-ending) to a complete parameter set (`'fatal'` scales every magnitude up), and each layer is individually switchable via a `PLAYER_DEATH_ENABLE_*` constant. All tuning lives in `src/vfx/playerDeathJuice.ts`; every juice object is tracked in a `playerDeathEffects` registry cleared on scene `SHUTDOWN`. Coverage: `src/vfx/playerDeathJuice.test.ts` plus per-path assertions in the `PlayScene`, `GymFormationScene` and `GymPowerUpsCombat` suites. See GDD §7.2–§7.3.
...
- Destruction SFX likewise vary between kills: the shared and Diver destruction sweeps apply one per-invocation pitch factor of ±15 % (`EXPLOSION_PITCH_JITTER` in `src/audio/effects.ts`) to all sweep endpoints. See GDD §7.3. Player destruction uses a dedicated layered `playPlayerDestructionSound()` cue (impact thump + descending body + noise tail) that fully replaces the generic enemy cue on the player-death paths; it is played exactly once by `spawnPlayerDeathJuice`.
...
- `src/powerups/effects.ts` — engine-agnostic active-effects registry: P5 timed +50% speed and rate of fire (refresh on re-collect, never additive), P8 lives (start 3, cap 5), P9 permanent magnet stacks (cap 5; radius `2× ship size + 50% per stack`; attraction at `MAGNET_ATTRACTION_SPEED`, slower than ship max speed).
- `src/powerups/icons.ts` — code-drawn neon icons shared by field drops and the HUD, plus the glowing drop bubble (`drawDropBubble`) and combined drop drawers (`drawPowerUpDrop`/`drawWeaponDrop`): every on-field drop renders a neon bubble (soft glow halo + crisp ring in the per-type aura colour) around its icon, `POWER_UP_DROP_SIZE` set to **16 px** (half the initial 32 px doubling; `POWER_UP_BUBBLE_*` constants tunable in `src/core/constants.ts`). The bubble is also the pickup boundary: the shared `dropBubbleRadius()` / `dropCollectRadius()` helpers derive both the drawn ring and the collision radius from the same `POWER_UP_BUBBLE_RADIUS_FACTOR`, so the ship collects a drop as soon as its hull touches the visible ring (AH-0MTVYCM2N002NKE4).
...
- `src/scenes/gym/GymPowerUpsUtility.ts` — the scene: round-robin spawning via `RoundRobinSpawner` (one drop per 5 s, 5 s lifetime → the next spawn coincides with the previous despawn), per-drop Graphics (glowing bubble + icon) scaled with the grow/hold/shrink lifecycle and destroyed on collect/despawn, overlap collection gated at >3% scale (pickup radius = ship hull + the visible bubble, `POWER_UP_DROP_SIZE × POWER_UP_BUBBLE_RADIUS_FACTOR × scale` = **32.4 px at full scale**), magnet attraction, live P5 speed and fire-rate multipliers on the ship, HUD attachment, and the shared "← INDEX" back button.
...
- `src/powerups/icons.ts` — distinctive code-drawn weapon icons (fan arc for Spread, parallel bars for Dual, waveform for Rapid, return/undo arrow for Reset) with the same **glowing drop bubble** as non-combat drops (`drawWeaponDrop` lays the per-type neon bubble under the icon; enlarged to `WEAPON_DROP_SIZE` = 16 px, which is also the weapon-drop collection boundary via the shared `dropCollectRadius()` helper); `src/audio/effects.ts` — spawn/despawn/collection/weapon-change cues (Web Audio synthesis, safe no-op fallback).
- `src/scenes/gym/GymWeapons.ts` — the scene: ship + auto-fire + round-robin weapon-drop spawner (**Spread → Dual → Rapid → Reset**, one drop at a time, 7 s lifetime — parameterised via `WEAPON_DROP_LIFETIME`, sharing the `PowerUp` grow/hold/shrink lifecycle with the 5 s non-combat gym), per-drop Graphics (glowing bubble + icon) scaled with the lifecycle, collection gated at ≥3% scale with the same hull-touches-bubble radius as non-combat drops, **cumulative collection** (adds each weapon to the active set; `WEAPON_TIMEOUT_MS` = 10 s per weapon), Reset clearing all timed weapons, and the shared "← INDEX" back button.
...
- `src/entities/BaseEnemy.ts` — per-enemy mineral accounting (`collectMineral()` / `mineralCount`) and the 25–50 % re-drop on death (`mineralRedropCount()` / `spawnMineralDrops()`, delegating the scatter maths to `src/entities/Mineral.ts`); `Asteroid` overrides `collectMineral()` as a no-op.
- `src/scenes/core/mineralKillDrops.ts` — the **single shared kill-drop rule** (`resolveMineralKillDrops(scene, entity, rng)`): small asteroid → one mineral at the death site, large/medium → none, non-asteroid enemy → the configured 25–50 % re-drop scattered near the death site. Consumed by **both** `PlayScene` and `GymFormationScene`, so the game and the gyms cannot diverge (pinned by the repo-wide guard in `CombatScene.equivalence.test.ts`).
...
- `src/scenes/PlayScene.ts` — wires the full loop through the shared rule: small asteroid kills drop minerals, player/enemy overlap collection, enemy re-drop on death, the HUD hold bar (a fixed-length outline that fills as the hold grows), and the hold-full choice.
- `src/scenes/gym/GymMinerals.ts` — the **asteroids-only** gym demonstrating the whole mechanic (auto-discovered as the "Minerals" index entry), now dropping through the same shared rule; every formation gym also seeds **100 random minerals** on create (`GymFormationScene`) and drops on kill via the shared rule.

Coverage: `src/entities/__tests__/Mineral.test.ts`, `src/entities/__tests__/BaseEnemy.minerals.test.ts`, `src/core/__tests__/GameState.minerals.test.ts`, `src/ui/__tests__/HUD.minerals.test.ts`, `src/powerups/__tests__/choice.test.ts`, `src/powerups/__tests__/effects.permanent.test.ts`, `src/scenes/__tests__/MineralChoiceScene.test.ts`, `src/scenes/__tests__/PlayScene.minerals.test.ts`, `src/scenes/core/mineralKillDrops.test.ts` and `src/scenes/gym/GymMinerals.test.ts`.
