...
The scene is reachable from the gym index ("Scout" entry) and returns to it via the "← INDEX" button — it is a gym testbed: the keyboard-controlled player ship (arrows + WASD) is present with live combat (player bullets destroy scouts; scout hits trigger explosion + respawn, infinite lives), but there is no HUD score, no power-ups, and no other enemies. Coverage: `src/entities/Scout.test.ts` + `src/scenes/gym/GymScout.test.ts` verify formation geometry, movement, explosion, shoot toggle and aimed-bullet behaviour.
